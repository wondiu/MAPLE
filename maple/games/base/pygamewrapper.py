...
class PyGameWrapper(object):
    ...
    n_agents: int
        The number of agents/players in the game

    actions_set: list of list of int
        Contains possible actions that the game responds too.
    """

    def __init__(self, width, height, n_agents, actions_set):

        # Required fields
        assert(n_agents==len(actions_set))
        self.n_agents = n_agents  # number of agents
        self.actions_set = actions_set  # holds supported actions
        self.current_actions = [0]*self.n_agents  # default to first action which should be noop
